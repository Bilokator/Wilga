...
begin
  ...
  begin
    ...
    gTintCtx := WilgaInitContextFromCanvas(gTintCanvas);
  end;

  // zamiast porównywać width/height → zawsze użyj bezpiecznego resize
...
begin
  ...
  // zamiast gCanvas.width / height pobieramy bezpiecznie rozmiar:
  asm
    var cnv = gCanvas;
    var off = cnv && cnv.__wilgaOffscreen ? cnv.__wilgaOffscreen : null;
    cw = (off ? off.width : cnv.width);
    ch = (off ? off.height : cnv.height);
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      gCtx.fillStyle := ColorToCanvasRGBA(currentColor);
      first := False;
    end;
...
var
  ...
  cw: Integer;
begin
  if not WilgaEnsureCanvasRef then Exit(0);

  cnv := gCanvas;
  offObj := nil;

  if JS.toBoolean(TJSObject(cnv)['__wilgaOffscreen']) then
    offObj := TJSObject(TJSObject(cnv)['__wilgaOffscreen']);

  if offObj <> nil then
    cw := Integer(offObj['width'])
  else
    cw := cnv.width;

  if gDPR = 0 then gDPR := window.devicePixelRatio;
...
begin
  ...
  img.onload := TJSEventHandler(
    ...
    begin
      ...
      // klasycznie ustawiamy rozmiar PRZED pobraniem kontekstu:
      tex.canvas.width  := img.width;
      tex.canvas.height := img.height;

      // TU: klasyczny kontekst 2D (NIE Offscreen)
...
function  LoadImageFromURL(const url: String): TTexture; overload; deprecated '⚠ użycie bez callbacka może nie działać — dodaj OnReady (callback).';
var
  img: TJSHTMLImageElement;
  tex: TTexture;
begin
  ...
  img.onload := TJSEventHandler(
    ...
    begin
      ...
      tex.canvas.width  := img.width;
      tex.canvas.height := img.height;
      ctx := WilgaInitContextFromCanvas(tex.canvas);
...
// Pełny tint RGBA z offscreenem (multiply + destination-in) – wersja offscreen-safe
...
var
  ...
  oldComp: String;
  useTint: Boolean;
  // przeskalowany origin (src-space → dst-space)
  scaleX, scaleY, ox, oy: Double;
  tw, th: Integer;
begin
  ...
  // origin podajemy w texelach źródła → przeskaluj do przestrzeni dst
  scaleX := dst.width  / sw;
  scaleY := dst.height / sh;
  ox := origin.x * scaleX;
  oy := origin.y * scaleY;
  ...
  try
    ...
    if useTint then
    begin
      // offscreen = dokładny rozmiar fragmentu źródła
      tw := Round(sw);
      th := Round(sh);
      EnsureTintCanvas(tw, th);

      // przygotowanie i kolorowanie
      gTintCtx.setTransform(1,0,0,1,0,0);
      gTintCtx.globalAlpha := 1.0;
      gTintCtx.globalCompositeOperation := 'source-over';
      gTintCtx.clearRect(0, 0, tw, th);
      ...
      // UWAGA: odejmujemy przeskalowany ox/oy (dst-space)
      gCtx.drawImage(gTintCanvas, 0, 0, sw, sh, -ox, -oy, dst.width, dst.height);
    end
    else
    begin
      gCtx.drawImage(tex.canvas, sx, sy, sw, sh, -ox, -oy, dst.width, dst.height);
    end;
...
procedure SetWindowSize(width, height: Integer);
begin
  // Ustaw CSS rozmiar – nadal działa, nawet po transferze
  gCanvas.style.setProperty('width',  IntToStr(width) + 'px');
  gCanvas.style.setProperty('height', IntToStr(height) + 'px');

  // Bezpieczna zmiana rzeczywistego rozmiaru bufora (uwzględnia Offscreen)
  WilgaResize(gCanvas, width, height, gDPR);

  // Jeśli Twoja WilgaResize już ustawia transform, poniższa linia niepotrzebna,
  // ale jeśli chcesz zachować jawne ustawienie:
  if gCtx <> nil then
    gCtx.setTransform(gDPR, 0, 0, gDPR, 0, 0);
end;
...
begin
  ...
  // TextMetrics jako JSValue
  m := gCtx.measureText(text);

  // pobieramy właściwości przez TJSObject(...)[...]
  ...
  // fallback, gdy przeglądarka nie raportuje bboxów
  if (asc = 0.0) and (desc = 0.0) then
    Result := size * 1.2
  else
    Result := asc + desc;
...
begin
  ...
  // Map#set wywołane bezpośrednio w JS
  asm
    GTextWidthCache.set(key, Result);
  end;
end;
...
begin
  ...
  // zapis do Map#set przez JS (unikamy brakującej metody Pascalowej)
  asm
...
procedure CanvasSave; begin gCtx.save; end;
procedure CanvasRestore; begin gCtx.restore; end;
...
procedure CanvasSetFillColor(const C: TColor);
begin
  gCtx.fillStyle := ColorToCanvasRGBA(C);
end;
...
begin
  case mode of
    bmNormal:    gCtx.globalCompositeOperation := 'source-over';
    bmAdd:       gCtx.globalCompositeOperation := 'lighter';
    bmMultiply:  gCtx.globalCompositeOperation := 'multiply';
    bmScreen:    gCtx.globalCompositeOperation := 'screen';
  end;
end;
...
var d: Double;
begin
  if DPR <= 0 then d := window.devicePixelRatio else d := DPR;
  ...
  if gCtx <> nil then
    gCtx.setTransform(d,0,0,d,0,0); // HiDPI, 1 CSS px = 1 jednostka
end;
...
begin
  asm
    ...
    if (cnv && typeof cnv.transferControlToOffscreen === 'function') {
      try {
        var off = cnv.transferControlToOffscreen();
        cnv.__wilgaOffscreen = off;          // <— zapamiętaj OffscreenCanvas
    ...
    if (!ctx2d && cnv && typeof cnv.getContext === 'function') {
      ctx2d = cnv.getContext('2d');         // fallback: zwykły 2D
    }
