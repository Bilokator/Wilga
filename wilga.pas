...
begin
  ...
  // każdy odcinek osobno — zawsze domknięte
  for i := 0 to High(pts) - 1 do
    DrawDashedLine(
      pts[i].x, pts[i].y,
      pts[i+1].x, pts[i+1].y,
      color, thickness, dashLen, gapLen
    );

  // ostatni odcinek → pierwszy
  DrawDashedLine(
    pts[High(pts)].x, pts[High(pts)].y,
    pts[0].x,         pts[0].y,
    color, thickness, dashLen, gapLen
  );
end;
...
begin
  // p1 -> p2
  DrawDashedLine(tri.p1.x, tri.p1.y, tri.p2.x, tri.p2.y,
    color, thickness, dashLen, gapLen);

  // p2 -> p3
  DrawDashedLine(tri.p2.x, tri.p2.y, tri.p3.x, tri.p3.y,
    color, thickness, dashLen, gapLen);

  // p3 -> p1
  DrawDashedLine(tri.p3.x, tri.p3.y, tri.p1.x, tri.p1.y,
    color, thickness, dashLen, gapLen);
end;
procedure DrawDashedCircle(cx, cy, radius: Double; const color: TColor;
...
var
  ...
  arr      : TJSArray;
begin
  if (radius <= 0) or (segments <= 0) then Exit;

  step := 2 * Pi / segments;

  CanvasSave;
  // ustawiamy pattern kreskowania
  arr := TJSArray.new;
  arr.push(dashLen);
  arr.push(gapLen);
  gCtx.setLineDash(arr);
...
var
  ...
  arr      : TJSArray;
begin
  if (rx <= 0) or (ry <= 0) or (segments <= 0) then Exit;
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        // PEŁNY TINT (bez rotacji)

        // 1) sprite z aktualnym blend mode
        WSetAlpha(1.0);
        gCtx.drawImage(tex.canvas, sx, sy, sw, sh, dx, dy, dw, dh);

        // 2) na to kolor tintu przez source-atop
        WSetAlpha(tint.a / 255.0);
        WSetOp('source-atop');
        gCtx.fillStyle :=
          'rgb(' + IntToStr(tint.r) + ',' + IntToStr(tint.g) + ',' + IntToStr(tint.b) + ')';
        gCtx.fillRect(dx, dy, dw, dh);

        // 3) powrót: alfa reset + przywrócenie blend mode
        WSetAlpha(1.0);
        SetBlendMode(gCurrentBlendMode);
      end;
    ...
    try
      ...
      begin
        // PEŁNY TINT z obrotem

        // 1) sprite z aktualnym blend mode
        WSetAlpha(1.0);
        gCtx.drawImage(tex.canvas, sx, sy, sw, sh, 0, 0, sw, sh);

        // 2) tint przez source-atop na obszarze sprite'a (0..sw,0..sh)
        WSetAlpha(tint.a / 255.0);
        WSetOp('source-atop');
        gCtx.fillStyle :=
          'rgb(' + IntToStr(tint.r) + ',' + IntToStr(tint.g) + ',' + IntToStr(tint.b) + ')';
...
begin
  ...
  // 1) Skasuj stringowy cache shadow-state,
  //    żeby kolejne WSetFill('rgba(...)') na pewno się przepchnęło.
  WSetFill('');

  // 2) Ustaw gradient BEZPOŚREDNIO na gCtx – omijamy decodeStyle(...) || 'black'
  gCtx.fillStyle := grad;
      ...
      if (!ctxProxy) {
        (function() {
          ...
          var methods = [
            ...
            "beginPath","closePath","moveTo","lineTo","arc","arcTo","rect","ellipse",
            "fill","stroke","clip",
